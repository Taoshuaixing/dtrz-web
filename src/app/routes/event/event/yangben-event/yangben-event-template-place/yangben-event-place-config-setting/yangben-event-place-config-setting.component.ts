...
@Component({
  selector: 'app-yangben-event-place-config-setting',
  templateUrl: './yangben-event-place-config-setting.component.html',
  styles: [],
})
export class YangbenEventPlaceConfigSettingComponent implements OnInit {
  isVisible = false;
  isOkLoading = false;
  fieldList :any;
  placeFieldList = [];
  ...
  checkedList = [];
  fixedFieldList= [
    {id:-1,fieldName:'事件名称',checked:true},
    {id:-2,fieldName:'事件分类',checked:true}
  ];
  ...
  async showModal() {
    ...
    if (this.parent) {
      this.placeFieldList = await new Promise((resolve, reject) => {
        this.crudService.get(environment.baseUrl + 'api', 'yangben-event-template-place', this.parent.id)
          .subscribe((res_1: any) => {
            resolve(res_1.yangbenEventTemplatePlaceFields);
          });
      });
    }
  ...
  async handleOk() {
    ...
    await new Promise((resolve, reject) => {
      this.crudService
        .add(environment.baseUrl + 'api', 'yangben-event-template-place/settings',{

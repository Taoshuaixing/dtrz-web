...
export class ListPersonComponent implements OnInit {
   ...
   */
  confirm(id) {
    this.crudService.delete(environment.baseUrl + 'api/category/' + id).subscribe((res) => {
  ...
  alter(data) {
    this.crudService
      .patch(environment.baseUrl + 'api/category/' + data.id, {name: data.name})
      .subscribe((res) => {
  ...
  async checkeParent() {
    const resA: [] = await new Promise((resolve, reject) => {
      this.crudService
        .search(environment.baseUrl + 'api', 'category', {filter: [`parent.id||$isnull`]})
        .subscribe((res: []) => {
          resolve(res);
        });
    });
    ...
    if (resA && resA.length === 0) {
      await new Promise((resolve, reject) => {
        this.crudService
          .add(environment.baseUrl + 'api', 'category', {
            id: 1,
            parent: null,
            name: '管理类别',
          })
  ...
  getData(e: any, type: string) {
    ...
    this.crudService.search(environment.baseUrl + 'api', 'article/searchWithCategoryCount', this.searchParams)
      .subscribe((res: any) => {
        this.loading = false;
        this.data = res.data;
    ...
    if (params) {
      ...
      const res = await new Promise((resolve, reject) => {
        for (const id of params) {
          this.crudService.del(environment.baseUrl + 'api', 'article', id).subscribe((res_articles) => {
  ...
  imgClick(item: any) {
    this.router.navigate(['/person/readonly-detail/' + item.id]);
  }

  getEmitter(event: any) {
    this.getData('', '');
    window.open('/#/person/detail/' + event.personId);
  }
  ...
  downLoad(type: string) {
    ...
    if (ids.length === 0) {
      this.msg.create('info', '请选择下载人物');
      this.loading = false;
      return;
    } else {
      if (type === 'doc') {
        this.download('api/document/downloadToDoc', ids.join(','), type);
      } else if (type === 'pdf') {
        this.download('api/document/downloadToPdf', ids.join(','), type);
      }
    }
  }

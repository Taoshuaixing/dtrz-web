...
@Component({
  selector: 'app-edit-website-base',
  templateUrl: './edit-website-base.component.html',
  styles: [
      `
      .big {
        width: 100%;
...
export class EditWebsiteBaseComponent implements OnInit {
  ...
  async handleOk() {
    ...
    const res: any = await new Promise((resolve, reject) => {
      ...
      this.crudService.add(this.baseUrl + 'api', 'website', addParams).subscribe((res_articles) => {
        resolve(res_articles);
      });
    });

    const websiteId = res.id;
    if (!websiteId) {
      this.msgSrv.error('保存失败');
      return;
    }
    this.msgSrv.success('保存成功');
    // window.open('/#/website/detail/' + websiteId);
    this.editorOk.emit({title: this.title, websiteId});
    this.isVisible = false;
  }

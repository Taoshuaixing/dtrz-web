...
export class NzModalUpdateDetailComponent implements OnInit {
  ...
  attachmentActionUrl= environment.baseUrl + 'api/yangben-attachment/upload';
  fileList: any=[];
  curRole: any;
 ...
 async searchFieldList(){
    ...
    if (this.adminFlag ===1 && this.shareFlag===-1) {
      ...
      //     })
      //   });
      const filedList_post: any = (await this.yangbenService.getShowItem_post(this.postId)).fieldList;
      ...
      this.fieldList.map(async m => {
        return (async () => {
          if (m.yangbenOptionConfig) {
            const data = await this.yangbenService.getTableData(m.yangbenOptionConfig.id,this.postId);
  ...
  async submit() {
    ...
    this.isChange = true;
      ...
      await new Promise((resolve, reject) => {
        this.crudService.get(environment.baseUrl + 'api', 'yangben-data/getOne', this.recUuid).subscribe((res: any) => {
          this.crudService
            .put(environment.baseUrl + 'api', 'yangben-data', {
              rec: { ...res.rec,
            ...
            .subscribe(res1 => {
              ...
              this.crudService.search(environment.baseUrl + 'api', 'yangben-data', {
  ...
  async validateFieldList(fieldList: any[]) {
    ...
    if (md5Str) {
      ...
      const res = await new Promise((resolve, reject) => {
        this.crudService.search(environment.baseUrl + 'api', 'yangben-data-rec', {
          filter,
        }).subscribe((res: any[]) => {
 ...
 async getData(falg: string) {
  ...
  upLoadChange($event) {
    ...
    if (
      ...
      this.fileList = this.fileList.map(m=>{
        if(m.response && m.response.attachmentId){
          return{
            ...
            name: m.name,
            status: 'done',
            url: `${environment.baseUrl}api/yangben-attachment/download/${m.response.attachmentId}`,
            thumbUrl: `${environment.baseUrl}api/yangben-attachment/download/${m.response.attachmentId}`,
          }
        }else{
          return m;
        }
      ...
      this.crudService.del(environment.baseUrl + 'api', 'yangben-attachment', attachmentId).subscribe(res => {

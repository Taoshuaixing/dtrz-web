...
@Component({
  selector: 'app-yangben-data-import',
  templateUrl: './yangben-data-import.component.html',
  styleUrls: ['./yangben-data-import.component.css'],
})
export class YangbenDataImportComponent implements OnInit, AfterViewInit, OnDestroy {
  constructor(
  ...
  async search() {
    ...
    this.listOfAllData = await new Promise((resolve, reject) => {
      this.crudService
        .search(environment.baseUrl + 'api', 'yangben-import-result', {
          filter: filter,
          limit: this.pageSize,
          page: this.pageIndex,
          sort: ['id,DESC'],
  ...
  async getOne(id: any) {
    return await new Promise((resolve, reject) => {
      this.crudService.get(environment.baseUrl + 'api', 'yangben-import-result', id).subscribe(async (res: any) => {
  ...
  getProgress(m: any) {
    return new Promise((resolve_1, reject_1) => {
      this.crudService
        .search(environment.baseUrl + 'api', 'yangben-import-progress', {
  ...
  del(id: any) {
    this.modalService.confirm({
      ...
      nzOkType: 'danger',
      nzOnOk: () => {
        this.crudService.del(environment.baseUrl + 'api', 'yangben-import-result', id).subscribe((res) => {
  ...
  async downloadErrorData(id: any) {
    this.excelService.download(
      'yangben',
      'api/yangben-data/downloadErrorData',
      {
        importId: id,
      },
      true,

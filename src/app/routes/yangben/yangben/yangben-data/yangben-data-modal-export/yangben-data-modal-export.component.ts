...
@Component({
  selector: 'app-yangben-data-modal-export',
  templateUrl: './yangben-data-modal-export.component.html',
  styles: [],
})
export class YangbenDataModalExportComponent implements OnInit {
  isVisible = false;
  isOkLoading = false;
  fieldList = [];
  selectedItem = [];
  ...
  async showModal(): Promise<void> {
    ...
    if (this.adminFlag === 1) {
      ...
      filedList_post = await new Promise((resolve, reject) => {
        this.crudService
          .search(environment.baseUrl + 'api', 'yangben-post-field-export', {
      ...
      if (this.shareFlag === 2) {
        ...
        this.fieldList = await new Promise((resolve, reject) => {
          this.crudService
            .search(environment.baseUrl + 'api', `yangben-field-define/fieldInfo/${this.postId}`, {})
            .subscribe((res: any[]) => {
              resolve(res);
            });
        });
  ...
  async export() {
    ...
    if (this.withAttach) {
      await this.fileDownloadService.download(
        'yangben' + '_' + this.datePipe.transform(new Date(), 'yyyyMMdd'),
        'zip',
        `api/yangben-data/export`,
        {

...
@Component({
  selector: 'app-yangben-data-sign-setting',
  templateUrl: './yangben-data-sign-setting.component.html',
  styles: []
})
export class YangbenDataSignSettingComponent implements OnInit {
  isVisible = false;
  isOkLoading = false;
  fieldList = [];
  ...
  async showModal(): Promise<void> {
    ...
    if (this.isPostUser) {
      // 岗位人员
      this.crudService
      .search(environment.baseUrl + 'api', 'yangben-post-field', { sort: [`id,ASC`], 
      filter:[
  ...
  async handleOk() {
    ...
    if (this.isPostUser) {
      await new Promise((resolve, reject) => {
        this.crudService
          .add(environment.baseUrl + 'api', 'yangben-post-field/setSignItem', {
            ids: ids,
            postId:this.postId,
            shareFlag:0
          })
      ...
      await new Promise((resolve, reject) => {
        this.crudService
          .add(environment.baseUrl + 'api', 'yangben-data/setSignItem', {
            ids: ids,
          })
          .subscribe(async res => {
            resolve(res);

...
@Component({
  selector: 'app-yangben-data',
  templateUrl: './yangben-data.component.html',
  styleUrls: ['./yangben-data.component.css'],
})
export class YangbenDataComponent implements OnInit, AfterViewInit {
  constructor(
  ...
  async ngOnInit() {
    ...
    this.activedrouted.queryParams.subscribe(async (params) => {
      if (params) {
        ...
        this.readonly = this.shareFlag > 0 || (this.adminFlag !== 1 && this.shareFlag === -1);
        ...
        this.chuzhiCntFieldId = params.chuzhiCntFieldId;
        this.chuzhiTimeFieldId = params.chuzhiTimeFieldId;
        this.chuzhiTime = params.chuzhiTime;
        this.chuzhiDuty = params.chuzhiDuty;
  ...
  async getSignList() {
    ...
    this.signList = await new Promise((resolve, reject) => {
      this.crudService
        .search(environment.baseUrl + 'api', 'yangben-data/getSignList', {
  ...
  async refreshField() {
      ...
      this.fieldList.map(async (m) => {
        return (async () => {
          if (m.yangbenOptionConfig) {
            const data = await this.yangbenService.getTableData(
              m.yangbenOptionConfig.id,
              this.shareFlag === -1 ? 0 : this.postId,
            );
    ...
    if (this.adminFlag || this.shareFlag === -1) {
    ...
    if (this.currentSys === 'dt') {
      ...
      if (chuzhiTimeField) {
        this.chuzhiTimeFieldId = chuzhiTimeField.id;
        tmpSearchItem = tmpSearchItem.filter((it) => it.id !== this.chuzhiTimeFieldId);
      }
    }
    ...
    const searchTime: any = [
      {
        id: this.currentSys === 'dt' ? this.chuzhiTimeFieldId : -1,
        fieldType: 3,
        fieldName: this.currentSys === 'dt' ? '处置时间' : '创建时间',
        showTime: true,
        name: this.currentSys === 'dt' ? null : 'createdTime',
        exp: 'between',
        val: dateRange,
        duty:
          this.currentSys === 'dt'
            ? [
                { value: 'm', id: 1, label: '白班', checked: false },
                { value: 'n', id: 2, label: '夜班', checked: false },
              ]
            : null,
      },
    ];
    ...
    if (!this.adminFlag && this.shareFlag === 0) {
      ...
      tmpSearchItem = [
        ...
        { id: -5, fieldType: 1, fieldName: '导入批次', name: 'batchNumber' },
    ...
    this.tableWidth = this.commonService.setColWidth(this.fieldListForList, this.content_width, 'content', 460);
  }
  ...
  async searchSub() {
    ...
    if (this.from === 'event' || this.from === 'option') {
      if (this.chuzhiCntFieldId && this.chuzhiCntFieldId > 0) {
        searchFilter = [...searchFilter, { fieldId: this.chuzhiCntFieldId, start: 1 }];
      }
      if (this.chuzhiTimeFieldId && this.chuzhiTimeFieldId > 0 && this.chuzhiTime && this.chuzhiTime.length === 2) {
        searchFilter = [
          ...searchFilter,
          {
            fieldId: this.chuzhiTimeFieldId,
            start: this.chuzhiTime[0],
            end: this.chuzhiTime[1],
            duty: this.chuzhiDuty,
          },
        ];
      }
    }
  ...
  deleteRec(recUuid: string) {
    if (recUuid) {
      this.modalService.confirm({
        ...
        nzOkType: 'danger',
        nzOnOk: async () => {
          this.crudService.del(environment.baseUrl + 'api', 'yangben-data', recUuid).subscribe((res) => {
            this.loading = false;
  ...
  async deleteYangbenList() {
    ...
    if (ids.length === 0) {
      ...
      this.modalService.confirm({
        nzTitle: '删除数据',
        nzContent:
          '<b style="color: red;">确定删除这些数据吗？</b> <br> <b>PS:若选择的删除数据中包含已分享的数据，分享数据将不会被删除！</b>',
        nzOkType: 'danger',
        nzOnOk: async () => {
          this.crudService
            .add(environment.baseUrl + 'api', 'yangben-data/removeBatch', {
              ids: ids,
              postId: this.postId,
            })
            .subscribe((res) => {
  ...
  changeNzOptions(): void {
    ...
    if (ids.length === 0) {
      ...
      this.modalService.confirm({
        ...
        nzOnOk: async () => {
          this.crudService
            .add(environment.baseUrl + 'api', 'yangben-data/updateBatch', {
              ids: ids,
              fieldId: fieldId,
              signValId: signValue,
  ...
  share() {
    ...
    if (ids.length === 0) {
      this.loading = false;
      this.msg.create('info', '请选择要分享的样本数据！');
      return;
    } else {
      this.modalService.confirm({
        nzTitle: '批量分享',
        ...
        nzOnOk: async () => {
          this.crudService
            .add(environment.baseUrl + 'api', 'yangben-post-data/sharePost', {
              ids: ids,
  ...
  cancelShareYangbenList() {
    ...
    if (ids.length === 0) {
      ...
      this.modalService.confirm({
        ...
        nzOnOk: async () => {
          this.crudService
            .add(environment.baseUrl + 'api', 'yangben-data/cancelShare', {
              ids: ids,
              postId: this.postId,
            })
            .subscribe((res) => {

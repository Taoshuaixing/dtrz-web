...
@Component({
  selector: 'app-yangben-similar-list',
  templateUrl: './yangben-similar-list.component.html',
  styleUrls: ['./yangben-similar-list.component.css'],
})
export class YangbenSimilarListComponent implements OnInit {
  ...
  async showModal(uuid:any ,similarCnt:any,sourceFlag:any){
    ...
    this.crudService.get(environment.baseUrl + 'api', 'yangben-field-define/fieldInfo', this.postId)
      .subscribe((res: any[]) => {
  ...
  async search(reset: boolean = false) {
      ...
      this.crudService.add(environment.baseUrl + 'api', 'yangben-data-tmp/getSimilarList', {

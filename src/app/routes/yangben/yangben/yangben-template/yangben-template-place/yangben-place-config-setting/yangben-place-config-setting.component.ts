...
@Component({
  selector: 'app-yangben-place-config-setting',
  templateUrl: './yangben-place-config-setting.component.html',
  styles: [],
})
export class YangbenPlaceConfigSettingComponent implements OnInit {
  isVisible = false;
  isOkLoading = false;
  fieldList :any;
  placeFieldList = [];
  ...
  async showModal() {
    ...
    if (this.parent) {
      this.placeFieldList = await new Promise((resolve, reject) => {
        this.crudService.get(environment.baseUrl + 'api', 'yangben-template-place', this.parent.id)
          .subscribe((res_1: any) => {
            resolve(res_1.yangbenTemplatePlaceFields);
          });
      });
    }
  ...
  async handleOk() {
    ...
    await new Promise((resolve, reject) => {
      this.crudService
        .add(environment.baseUrl + 'api', 'yangben-template-place/settings',{

...
@Component({
  selector: 'app-yangben-post-config-setting',
  templateUrl: './yangben-post-config-setting.component.html',
  styles: [],
})
export class YangbenPostConfigSettingComponent implements OnInit {
  isVisible = false;
  isOkLoading = false;
  fieldList = [];
  postFieldList = [];
  ...
  showModal(): void {
    ...
    this.fieldList = [];
    this.crudService
      .search(environment.baseUrl + 'api', 'yangben-template-place', { sort: [`id,ASC`] })
  ...
  async handleOk() {
    ...
    await new Promise((resolve, reject) => {
      this.crudService
        .add(environment.baseUrl + 'api', 'yangben-template-post',{
          placeIds: ids,

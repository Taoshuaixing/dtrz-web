...
@Component({
  selector: 'app-edit-zhanghao',
  templateUrl: './edit-zhanghao.component.html',
  styleUrls: ['./edit-zhanghao.component.less'],
})
export class EditZhanghaoComponent implements OnInit {
  constructor(
  ...
  async ngOnInit() {
    ...
    if (this.tempId) {
      const res: any = await new Promise((resolve, reject) => {
        this.crudService
          .get(environment.baseUrl + 'api', 'zhanghao-field-define', this.tempId)
          .subscribe((res1: any) => {
            resolve(res1);
          });
      });
      ...
      if (this.fieldType === 5 || this.fieldType === 6) {
        this.showFlag = true;
        if (res.zhanghaoOptionConfig) {
          this.fieldTypeValue = res.zhanghaoOptionConfig.id;
        }
      }
  ...
  async submit() {
    ...
    if (this.tempId) {
      ...
      this.crudService.get(environment.baseUrl + 'api', 'zhanghao-field-define', this.tempId).subscribe((res: any) => {
        this.crudService
          .update(environment.baseUrl + 'api', 'zhanghao-field-define', this.tempId, {
            ...
            zhanghaoOptionConfig: { id: this.fieldTypeValue },
          })
          .subscribe((res1) => {
            this.loading = false;
            this.back();
      ...
      // 新增
      this.crudService
        .add(environment.baseUrl + 'api', 'zhanghao-field-define', {
  ...
  back() {
    this.router.navigate(['/zhanghao/zhanghao-field-define']);
  }
  ...
  async validateFieldTypeValue(fieldTypeValue: number) {
    ...
    const res = await new Promise((resolve, reject) => {
      this.crudService
        .search(environment.baseUrl + 'api', 'zhanghao-field-define', {
          filter: [`zhanghaoOptionConfig.id||$eq||${fieldTypeValue}`],
